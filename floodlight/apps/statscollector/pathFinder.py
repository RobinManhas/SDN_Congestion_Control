...
def networkPoller():
	...
	while not stopFlag:
		...
		while counter < 4:
			...
			q_res = os.popen(queuecmd).read().split('\n')[2] # get bytes sent for interactive queue
			bytes_sent = ((q_res.split(" ")[6]).split("=")[1])[:-1]

			queuecmd = "sudo tc -s qdisc ls dev %s"%(ifaceList[counter])
			q_res = os.popen(queuecmd).read().split('\n')[2]
			bytes_buf = (q_res.split(" ")[2])[:-1]
			if counter == 2:
				interface_stats.extend([bytes_sent,bytes_buf,SWITCH_MIN_RATES[counter],50])
			else:
				interface_stats.extend([bytes_sent,bytes_buf,SWITCH_MIN_RATES[counter],0])
